...
import { switchNetwork } from "./factory";

const TIMEFRAME = 1;

export async function fetchCreateTokenEvents(tokenAddressParam = null, chain) {
  try {
    ...
    const eightHoursAgoTimestamp = latestBlockTimestamp - TIMEFRAME * 60 * 60;
      ...
      return Promise.all(
        allEvents.map(async (event) => {
          ...
          const tokenData = {
            ...
            ethPriceAtTime: await fetchEthPriceFromOracle(block.timestamp),
            chain: chain,
          };
...
export const getTokenBalance = async (tokenAddress, chain) => {
...
export const getEtherBalance = async (chain) => {
  try {
    await switchNetwork(chain);
    // Initialize the Web3 provider and signer
    const provider = new ethers.providers.Web3Provider(window.ethereum);
...
export async function fetchTokenBuysAndSells(tokenAddress = null, chain) {
  const factoryAddress =
    chain === "ethereum"
      ? process.env.NEXT_PUBLIC_FACTORY_ETH
      : process.env.NEXT_PUBLIC_FACTORY_BASE;
  ...
  try {
    // await deleteTransactionData(tokenAddress); // DEBUG

    const ethPriceAtTime = (await fetchEthPriceFromOracle()) || 0;
    ...
    const twoHoursAgoTimestamp = latestBlockTimestamp - TIMEFRAME * 60 * 60;
    ...
    for (let i = startBlock; i <= latestBlockNumber; i += stepSize) {
      ...
      for (const event of purchaseEvents) {
        ...
        await createNewTokenTransaction(eventData);
        //let res = await createNewTokenTransaction(eventData);
        //console.log(res);
      }
