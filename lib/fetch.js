...
export async function fetchCreateTokenEvents(tokenAddress = null) {
  try {
    ...
    const eightHoursAgoTimestamp = latestBlockTimestamp - 6 * 60 * 60;

    // Step backward to find the block mined approximately X hours ago
...
export async function fetchTokenBuysAndSells(tokenAddress = null) {
  try {
    ...
    // Calculate the timestamp for X hours ago (in seconds)
    const twoHoursAgoTimestamp = latestBlockTimestamp - 6 * 60 * 60;
