...
export const createToken = async (
  ...
  try {
    ...
    const tokenData = {
      ...
      ethPriceAtTime: await fetchEthPriceFromOracle(),
      chain: chain,
    };
...
export const createAndBuyToken = async (
  ...
  try {
    ...
    const {
      tokenAddress,
      name: tokenName, // Rename name to tokenName to avoid conflicts
      symbol,
      initialSupply,
      description: tokenDescription, // Rename description to tokenDescription
      imageUrl,
      twitterLink,
      telegramLink,
      websiteLink,
    ...
    // Save the token data to the database
    const tokenData = {
      tokenCreator: await signer.getAddress(),
      tokenFactory: factoryAddress,
      tokenAddress: tokenAddress,
      name: tokenName, // Use tokenName here
      symbol: symbol,
      initialSupply: initialSupply.toString(),
      description: tokenDescription, // Use tokenDescription here
      imageUrl: imageUrl,
    ...
    const ethPriceAtTime = await fetchEthPriceFromOracle();
...
export const buyToken = async (ethAmount, tokenAddress, chain) => {
  ...
  try {
    ...
    const ethPriceAtTime = await fetchEthPriceFromOracle(); // Implement this function using a pricing API
...
export const sellToken = async (tokenAmount, tokenAddress, chain) => {
  ...
  try {
    ...
    let currentAllowance = await token.allowance(
      walletAddress,
      factory.address
    );
