...
export const createToken = async (
  ...
  try {
    ...
    const factory = new ethers.Contract(
      "0xEB398Dc9cAB95acB5D564faec169548658251C1E",
      SimpleFactoryAbi,
      signer
    );
...
export const getRecentTokens = async () => {
  try {
    ...
    const factory = new ethers.Contract(
      "0xEB398Dc9cAB95acB5D564faec169548658251C1E", // Indirizzo del contratto factory
      SimpleFactoryAbi,
      signer
    );
...
export const buyToken = async (ethAmount, tokenAddress) => {
  ...
  try {
    ...
    const tx = await factory.buyToken(tokenAddress, { value: purchaseAmount });
...
export const sellToken = async (tokenAmount, tokenAddress) => {
  ...
  try {
    ...
    toast("Token Approved for Sale!");

    // Call the sellToken function on the factory contract to sell the approved tokens
    const sellTx = await factory.sellToken(tokenAddress, sellAmount);

    // Wait for the transaction to be mined
    await sellTx.wait();

    // Log the transaction hash and display success message
    console.log(sellTx.hash);
