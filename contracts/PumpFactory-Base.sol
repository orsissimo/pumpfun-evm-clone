...
contract PumpFactory is ReentrancyGuard {
    ...
    uint256 public ethCap = 0.5 ether;
    ...
    function addLiquidityAndBurn(address tokenAddress, uint256 ethToAdd) internal {
        PumpToken token = PumpToken(tokenAddress);
        TokenInfo storage tokenInfo = tokens[tokenAddress];
        address tokenCreator = tokenInfo.creator;
        ...
        emit LiquidityAdded(tokenAddress, ethToAddLiquidity, tokenFactoryBalance, tokensToBurn, tokenCreator, PUMP_STYLE_WEBSITE, PUMP_STYLE_X, PUMP_STYLE_TELEGRAM);
